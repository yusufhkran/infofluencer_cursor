...
import { registerInfluencer } from '../../services/auth';

const RegisterInfluencerPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await registerInfluencer(form);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-500 via-fuchsia-500 to-indigo-500 relative overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col justify-center w-1/2 pl-24 z-10">
        <h1 className="text-5xl font-extrabold mb-6 leading-tight text-white drop-shadow-lg">
          <span className="bg-gradient-to-r from-yellow-300 via-pink-400 to-fuchsia-500 bg-clip-text text-transparent">Influencer Kayıt</span>
        </h1>
        <p className="text-xl text-white/80 mb-8 max-w-lg drop-shadow">
          Kendi markanı yarat, topluluğunu büyüt, <span className="font-semibold text-yellow-200">influence</span> dünyasına katıl!
        </p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 z-10">
        <div className="w-full max-w-md bg-white/90 rounded-3xl shadow-2xl p-10 relative backdrop-blur-md border border-white/40">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-extrabold mb-2 bg-gradient-to-r from-pink-500 via-fuchsia-500 to-indigo-500 bg-clip-text text-transparent">Influencer Kayıt</h2>
            <p className="text-gray-500">Topluluğumuza katıl, hemen başla!</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-gray-700 font-semibold mb-1">Soyad</label>
              <input
                type="text"
                name="surname"
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <Link to="/influencer/login" className="text-pink-600 hover:underline font-semibold">Giriş Yap</Link>
          </div>
        </div>
      </div>
    </div>
