...
class InfluencerProfileAdmin(admin.ModelAdmin):
    list_display = ['user', 'first_name', 'last_name', 'email', 'instagram_handle', 'created_at']
    list_filter = ['created_at']
    search_fields = ['first_name', 'last_name', 'email', 'instagram_handle', 'user__email']
    readonly_fields = ['created_at']
    ...
    fieldsets = (
        ...
        ('Social Media', {
            'fields': ('instagram_handle', 'youtube_channel_id')
        }),
        ('Timestamps', {
            'fields': ('created_at',),
            'classes': ('collapse',)
